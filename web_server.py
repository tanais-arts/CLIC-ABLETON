...
<style>
  ...
  #sceneLabelNext.loop-active { color: #e0342b; }
  #sceneLabelNext.pulse {
    animation: sceneLabelNextPulse 300ms steps(1, end);
  }
  #sceneLabelNext.loop-active.pulse {
    animation: loopReprisePulse 300ms steps(1, end);
  }
  @keyframes sceneLabelNextPulse {
    from { color: #ffffff; }
    to { color: transparent; }
  }
  @keyframes loopReprisePulse {
    from { color: #e0342b; }
    to { color: transparent; }
  }
  ...
  #noSleepVideo {
    ...
    object-fit: cover; opacity: 0.001; pointer-events: none; z-index: 0;
  }
  #lyricsScroll {
    display: none;
...
<body>
  ...
  <div id="beat">
    ...
    <!-- Repli "no-sleep" (voir script plus bas) : boucle muette, invisible,
         qui garde l'écran allumé même quand l'API Wake Lock échoue (page
         servie en HTTP, pas dans un contexte sécurisé). -->
    <video id="noSleepVideo" muted playsinline webkit-playsinline loop autoplay>
...
// Empêche la mise en veille : le Wake Lock est redemandé au retour au premier plan.
let wakeLock = null;
const noSleepVideo = document.getElementById('noSleepVideo');
// Le lecteur ne dépend pas de #beat, masqué en mode Prompteur.
document.body.appendChild(noSleepVideo);
const mp4Source = noSleepVideo.querySelector('source[src^="data:video/mp4"]');
if (mp4Source) {
  noSleepVideo.src = mp4Source.src;
  noSleepVideo.load();
}
...
noSleepVideo.addEventListener('pause', () => {
  if (document.visibilityState === 'visible') setTimeout(keepScreenAwake, 250);
});
...
async function poll() {
  try {
    ...
    if (data.emergency) {
      ...
      nextLabelEl.classList.remove('hasNext', 'loop-active');
    } else {
      sceneLabelCurrentEl.textContent = data.scene_label || '';
      sceneLabelCurrentEl.classList.remove('emergency', 'pulse');
      const reprise = !!data.loop_active && !!data.loop_reprise;
      const nextLabel = data.loop_active ? (reprise ? 'REPRISE' : '') : data.next_scene_label;
      nextLabelEl.textContent = nextLabel ? (' ' + nextLabel) : '';
      nextLabelEl.classList.toggle('hasNext', !!nextLabel);
      nextLabelEl.classList.toggle('loop-active', reprise);
    }
...
<style>
  ...
  @-webkit-keyframes loopReprisePulse { from { color: #e0342b; } to { color: transparent; } }
  @keyframes loopReprisePulse { from { color: #e0342b; } to { color: transparent; } }
  #sceneLabelNext.loop-active.pulse {
    -webkit-animation: loopReprisePulse 300ms steps(1, end); animation: loopReprisePulse 300ms steps(1, end);
  }
...
// Repli fonctionnant même en HTTP, voir la même logique dans la page
// moderne ci-dessus.
var noSleepVideo = document.getElementById('noSleepVideo');
document.body.appendChild(noSleepVideo);
var mp4Source = noSleepVideo.querySelector('source[src^="data:video/mp4"]');
if (mp4Source) {
  noSleepVideo.src = mp4Source.src;
  noSleepVideo.load();
}
...
// iOS ignore de plus en plus une simple vidéo muette : un vrai geste de
// l'utilisateur autorise à activer sa piste audio (silence à -91dB,
// inaudible), qu'iOS traite alors comme une vraie lecture active.
...
function render(data) {
  ...
  if (data.connected) {
    ...
    if (data.beat !== lastBeat) {
      ...
      if (data.loop_reprise) {
        ...
        sceneLabelNextEl.classList.remove('pulse');
      } else {
        if (data.bpm) sceneLabelNextEl.style.animationDuration = (30000 / data.bpm) + 'ms';
        retriggerEl(sceneLabelNextEl, 'pulse');
      }
  ...
  if (data.emergency) {
    ...
    sceneLabelNextEl.classList.remove('hasNext', 'loop-active');
  } else {
    sceneLabelCurrentEl.innerHTML = data.scene_label ? data.scene_label : '';
    sceneLabelCurrentEl.style.color = '';
    var reprise = !!data.loop_active && !!data.loop_reprise;
    var nextLabel = data.loop_active ? (reprise ? 'REPRISE' : '') : data.next_scene_label;
    sceneLabelNextEl.innerHTML = nextLabel ? (' ' + nextLabel) : '';
    ...
    sceneLabelNextEl.classList.toggle('hasNext', !!nextLabel);
    sceneLabelNextEl.classList.toggle('loop-active', reprise);
  }
...
class SharedBeatState:
    ...
    def set_loop_reprise(self, active: bool) -> None:
        with self._lock:
            self._loop_reprise = active
    ...
    def compute(self, latency_ms: float = 0.0) -> dict:
        ...
        with self._lock:
            ...
            loop_reprise = self._loop_reprise
